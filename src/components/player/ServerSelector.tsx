...
function Badge({ badge }: { badge: ProviderBadge }) {
  ...
  const styles =
    badge === 'FAST'
      ...
      : badge === 'HD'
        ? 'bg-purple-300/15 text-purple-200 ring-purple-300/30'
        : 'bg-purple-400/15 text-purple-300 ring-purple-400/30';
  return (
    <span
  ...
  return (
    <button
      ...
      className={`group relative flex w-full items-center gap-3 rounded-xl border px-4 py-3.5 text-left transition ${
        isActive
          ? 'border-purple-400/60 bg-purple-500/10 shadow-[0_0_24px_rgba(139,92,246,0.2)]'
          : 'border-white/[0.06] bg-white/[0.03] hover:border-purple-500/30 hover:bg-white/[0.06]'
      }`}
    >
      {isActive && (
        <span className="absolute left-0 top-2 bottom-2 w-1 rounded-full bg-purple-400 shadow-[0_0_12px_rgba(139,92,246,0.8)]" />
      )}
      <span
        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${
          isActive ? 'bg-purple-500/20 text-purple-200' : 'bg-white/5 text-white/45'
        }`}
      >
      ...
      {isActive ? (
        <span className="shrink-0 text-[11px] font-bold uppercase tracking-wider text-purple-300">
          Active
        </span>
      ) : (
...
export function ServerSelector({ onSwitch }: ServerSelectorProps) {
  ...
  return (
    <section className="mx-auto mt-6 max-w-md px-4 sm:px-0">
      <div className="mb-3 flex items-start gap-3 rounded-xl border border-purple-500/25 bg-purple-950/40 px-3 py-2.5">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-purple-500/20 text-purple-400">
          ⚠
        </span>
        <div className="min-w-0 flex-1 text-sm">
          <p className="font-semibold uppercase tracking-wide text-purple-400/90">
            Not working?
          </p>
      ...
      <motion.div
        ...
        className="overflow-hidden rounded-2xl border border-purple-500/20 bg-gradient-to-b from-purple-600/10 via-surface-card to-surface-card shadow-card"
      >
        ...
        <div className="px-4 py-4">
          <p className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-purple-300">
