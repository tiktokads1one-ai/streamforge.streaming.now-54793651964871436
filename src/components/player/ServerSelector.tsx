...
function Badge({ badge }: { badge: ProviderBadge }) {
  ...
  const styles =
    badge === 'FAST'
      ...
      : badge === 'HD'
        ? 'bg-yellow-300/15 text-yellow-200 ring-yellow-300/30'
        : 'bg-yellow-400/15 text-yellow-300 ring-yellow-400/30';
  return (
    <span
  ...
  return (
    <button
      ...
      className={`group relative flex w-full items-center gap-3 rounded-xl border px-4 py-3.5 text-left transition ${
        isActive
          ? 'border-yellow-400/60 bg-yellow-500/10 shadow-[0_0_24px_rgba(255,215,0,0.2)]'
          : 'border-white/[0.06] bg-white/[0.03] hover:border-yellow-500/30 hover:bg-white/[0.06]'
      }`}
    >
      {isActive && (
        <span className="absolute left-0 top-2 bottom-2 w-1 rounded-full bg-yellow-400 shadow-[0_0_12px_rgba(255,215,0,0.8)]" />
      )}
      <span
        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${
          isActive ? 'bg-yellow-500/20 text-yellow-200' : 'bg-white/5 text-white/45'
        }`}
      >
      ...
      {isActive ? (
        <span className="shrink-0 text-[11px] font-bold uppercase tracking-wider text-yellow-300">
          Active
        </span>
      ) : (
...
export function ServerSelector({ onSwitch }: ServerSelectorProps) {
  ...
  return (
    <section className="mx-auto mt-6 max-w-md px-4 sm:px-0">
      <div className="mb-3 flex items-start gap-3 rounded-xl border border-yellow-500/25 bg-yellow-950/40 px-3 py-2.5">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-yellow-500/20 text-yellow-400">
          ⚠
        </span>
        <div className="min-w-0 flex-1 text-sm">
          <p className="font-semibold uppercase tracking-wide text-yellow-400/90">
            Not working?
          </p>
      ...
      <motion.div
        ...
        className="overflow-hidden rounded-2xl border border-yellow-500/20 bg-gradient-to-b from-yellow-600/10 via-surface-card to-surface-card shadow-card"
      >
        ...
        <div className="px-4 py-4">
          <p className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-yellow-300">
